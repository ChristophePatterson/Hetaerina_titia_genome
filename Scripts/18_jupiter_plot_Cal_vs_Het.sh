...
/home/tmjj24/apps/JupiterPlot/jupiter name=IolEle1.2_vs_HetTit1.0_v7\
	ref=$IolEle \
	fa=$HetTit \
...
# Create files for input into syntenyPlotteR

bioawk -c fastx '{ print $name, length($seq), "IolEle" }' $IolEle > Chromosome_length_HetTit_IolEle.txt
bioawk -c fastx '{ print $name, length($seq), "HetTit" }' $HetTit >> Chromosome_length_HetTit_IolEle.txt
