...
input.file <- "IolEle1.2_vs_HetAmer1.0"
...
# Simple plot
chrom_length <- read.table(paste0("Data/Jupiter/Chromosome_length_HetAmer_IolEle.txt"))
names(chrom_length) <- c("chr", "length", "Spp")
chrom_length <- chrom_length[!grepl(chrom_length$chr, pattern = "CAKLCU02")|chrom_length$Spp=="HetAmer",]
...
col.data.spp <- cbind(c(spp.1, spp.2), c("lightblue","darkolivegreen3"))
...
circos.track(ylim = c(0, 1), panel.fun = function(x, y) {
  ...
  circos.rect(xlim[1], 0, xlim[2],1, lwd = 1, col = chrom$spp.col[which(chrom$chr==CELL_META$sector.index)])
  
}, track.height = 0.15,bg.border = NA, bg.lwd =1)
...
highlight.chromosome(chrom$chr[chrom$chr=="HetAmer-chr1"], 
                     col = "darkolivegreen3", track.index = 2, padding = c(1,0,1,0))
highlight.chromosome(c("IolEle-chr12", "IolEle-chr9"), 
